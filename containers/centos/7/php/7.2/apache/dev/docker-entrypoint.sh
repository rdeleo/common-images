...
# Init xdebug
sed -e '~__XDEBUG_ID_KEY__~'${XDEBUG_ID_KEY}'~g' \
    -e '~__XDEBUG_DEFAULT_ENABLE__~'${XDEBUG_DEFAULT_ENABLE}'~g' \
    -e '~__XDEBUG_REMOTE_LOG__~"'${XDEBUG_REMOTE_LOG}'"~g' \
    -e '~__XDEBUG_REMOTE_ENABLE__~'${XDEBUG_REMOTE_ENABLE}'~g' \
    -e '~__XDEBUG_REMOTE_AUTOSTART__~'${XDEBUG_REMOTE_AUTOSTART}'~g' \
    -e '~__XDEBUG_REMOTE_CONNECT_BACK__~'${XDEBUG_REMOTE_CONNECT_BACK}'~g' \
    -e '~__XDEBUG_REMOTE_PORT__~'${XDEBUG_REMOTE_PORT}'~g' \
    -e '~__XDEBUG_PROFILER_ENABLE__~'${XDEBUG_PROFILER_ENABLE}'~g' \
    -e '~__DOCKER_HOST_IP__~'${DOCKER_HOST_IP}'~g' \
    ${TEMPLATE_DIR}/${XDEBUG_INI} > ${PHPD_DIR}/${XDEBUG_INI}

# Init apache
sed -e '~__APP_DOMAIN_NAME__~'${APP_DOMAIN_NAME}'~g' \
    -e '~__APACHE_SERVER_ADMIN_EMAIL__~'${APACHE_SERVER_ADMIN_EMAIL}'~g' \
    -e '~__APACHE_DOCUMENT_ROOT__~"'${APACHE_DOCUMENT_ROOT}'"~g' \
    -e '~__APACHE_ERROR_LOG__~'${APACHE_ERROR_LOG}'~g' \
